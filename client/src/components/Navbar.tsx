...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { href: '#awards', label: 'REGISTRY', sector: 'SEC_05', icon: <Award className="w-4 h-4" /> },
    { href: '#experience', label: 'AUDIT', sector: 'SEC_06', icon: <ChartSpline className="w-4 h-4" /> },
    { href: '#techstack', label: 'MAINFRAME', sector: 'SEC_07', icon: <Cpu className="w-4 h-4" /> }
  ];
  ...
  return (
    <>
    <header className="fixed top-0 left-0 w-full z-[150] pointer-events-none bg-[#060608] md:bg-black/95 md:backdrop-blur-md border-b-2 border-white/5 pb-2 md:pb-4 shadow-[0_15px_30px_rgba(0,0,0,0.8)]">
        ...
        <div className="flex justify-between items-center pointer-events-auto">
          ...
          <div className="bg-black/90 border-2 border-primary/20 p-5 flex items-center gap-5 group hover:border-primary transition-all pointer-events-auto relative shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex flex-col">
              <span className="text-sm md:text-lg font-black tracking-tighter text-white leading-none italic uppercase">PIYUSH_KB</span>
              <span className="text-[8px] md:text-[10px] font-mono text-primary font-bold mt-1 uppercase tracking-widest md:tracking-[0.3em]">ENGINEERING_v5.0</span>
            </div>
          </div>
      ...
      <AnimatePresence>
        {drawerOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            style={{ zIndex: 999999, backgroundColor: '#060608' }}
              ...
              <div className="flex flex-col gap-4">
                {navItems.map(({ href, label, icon }) => (
                  <a
                    ...
                    className="flex items-center gap-4 p-4 border border-white/5 hover:border-primary/50 text-lg md:text-2xl font-bold transition-all group"
                  >
                    <span className="text-primary group-hover:scale-110 transition-transform">{icon}</span>
                    <span className="group-hover:translate-x-2 transition-transform truncate">{label}</span>
                  </a>
                ))}
              </div>
            </div>
